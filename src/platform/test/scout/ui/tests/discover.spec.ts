...
test.describe('discover test', {tag: ['@ess']}, () => {
  ...
  test.describe('query', () => {
    ...
    test('should show the correct hit count', async ({ pageObjects }) => {
      ...
      await expect.poll(async () => await pageObjects.discover.getHitCount()).toBe(expectedHitCount);
    });
  ...
  test.describe('data-shared-item', () => {
    test('should have correct data-shared-item title and description', async ({ pageObjects }) => {
      const expected = {
      ...
      const { title, description } = await pageObjects.discover.getSharedItemTitleAndDescription();
